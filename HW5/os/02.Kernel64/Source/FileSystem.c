...
BOOL kCreateDirectory(const char *pcDirName, DIRECTORYENTRY *pstEntry, int *piDirectoryEntryIndex)
{
	DWORD dwCluster;
	int iDirNameLength;

	iDirNameLength = kStrLen(pcDirName);
	if ((iDirNameLength > (sizeof(pstEntry->vcFileName) - 1)) ||
		(iDirNameLength == 0))
	{
		return NULL;
	}
...
DIR *kOpenDirectory(const char *pcDirectoryName)
{
	...
	iDirectoryEntryOffset = kFindDirectoryEntry(pcDirectoryName, &pstDirectoryBuffer);
