...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
    {
        ...
        {"mkdir", "Create Directory, ex)mkdir temp", kCreateDirectoryInRootDirectory},
        {"rmdir", "Delete Directory, ex)rmdir temp", kDeleteDirectoryInRootDirectory},
...
static void kCreateDirectoryInRootDirectory(const char *pcParameterBuffer)
{
    ...
    iDirectoryEntryOffset = kFindDirectoryEntry(vcDirName, &stEntry);
    if (iDirectoryEntryOffset == -1)
    {
        pstDirectory = makedir(vcDirName, &stEntry, &iDirectoryEntryOffset);
    }
    ...
    pstDirectory->stFileHandle.iDirectoryEntryOffset = iDirectoryEntryOffset;
    pstDirectory->stFileHandle.dwStartClusterIndex = stEntry.dwStartClusterIndex;
    pstDirectory->stFileHandle.dwCurrentClusterIndex = stEntry.dwStartClusterIndex;
    pstDirectory->stFileHandle.dwPreviousClusterIndex = stEntry.dwStartClusterIndex;
    pstDirectory->stFileHandle.dwCurrentOffset = 0;

    kCloseDirectory(pstDirectory);
...
static void kDeleteDirectoryInRootDirectory(const char *pcParameterBuffer)
{
    PARAMETERLIST stList;
    char vcDirName[50];
    int iLength;
    ...
    if (kRemoveDirectory(vcDirName) != 0)
    {
...
static void kShowRootDirectory(const char *pcParameterBuffer)
{
    ...
    while (1)
    {
        ...
        vcBuffer[kStrLen(pstEntry->d_name) + 60] = '\0';

        switch (pstEntry->iFlag)
        {
        case 0:
...
static void kChangeDirectory(const char *pcParameterBuffer)
{
    ...
    if (kGetNextParameter(&stList, vcBuffer) == 0)
    {
        kPrintf("ex)cd temp\n");
        return;
    }

    iDirectoryEntryOffset = kFindDirectoryEntry(vcBuffer, &stEntry);
    kGetDirectoryEntryData(iDirectoryEntryOffset, &stEntry);

    if (kMemCmp(vcBuffer, "..", 2) == 0)
    {
        vcDirectoryBuffer[0] = '\0';
    }
    else if (kMemCmp(vcBuffer, ".", 1) == 0)
    {
    }
    else if (kMemCmp(vcBuffer, stEntry.vcFileName, kStrLen(stEntry.vcFileName)) == 0 && (stEntry.iFlag == 1))
    {
        for (int i = 0; i < sizeof(vcDirectoryBuffer); i++)
        {
            vcDirectoryBuffer[i] = '\0';
        }
        kMemCpy(vcDirectoryBuffer, vcBuffer, kStrLen(vcBuffer));
    }
    else
    {
