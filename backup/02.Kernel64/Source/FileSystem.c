...
BOOL kFormat( void )
{
   ...
   kReadRTCDate( &wYear, &bMonth, &bDayOfMonth, &bDayOfWeek );
    ...
    kSetDotInDirectory( bSecond, bMinute, bHour, bDayOfWeek, bDayOfMonth, bMonth, wYear);
...
void kSetDotInDirectory(BYTE bSecond, BYTE bMinute, BYTE bHour, BYTE bDayOfWeek, BYTE bDayOfMonth, BYTE bMonth,WORD wYear){
        ...
        int* piDirectoryEntryIndex )
{
    ...
    // ºó Å¬·¯œºÅÍžŠ Ã£ŸÆŒ­ ÇÒŽçµÈ °ÍÀž·Î Œ³Á€
    dwCluster = kFindFreeCluster();
    if( ( dwCluster == FILESYSTEM_LASTCLUSTER ) ||
        ( kSetClusterLinkData( dwCluster, FILESYSTEM_LASTCLUSTER ) == FALSE ) )
    {
