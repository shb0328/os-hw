...
#ifndef O_BINARY
#define O_BINARY 0x00 // in linux, O_BINARY not exist
#endif
...
int main(int argc, char* argv[])
{
    ...
    if( argc < 3 )
    {
        fprintf( stderr, "[ERROR] ImageMaker.exe BootLoader.bin KernelLoader.bin Kernel32.bin Kernel64.bin\n" );
        exit( -1 );
    }
    ...
    //--------------------------------------------------------------------------
    // 커널 로더 파일을 열어 모든 내용을 디스크 이미지에 복사
    //--------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    //64 비트 커널 파일올 열어서 모든 내용을 디스크 이미지 파일로 복사
    //-------------------------------------------------------------------------
...
void WriteKernelInformation( int iTargetFd, int iTotalKernelSectorCount, int iKernel32SectorCount)
{
    ...
    lPosition = lseek( iTargetFd, 5 + 512, SEEK_SET );
    if( lPosition == -1 )
    {
